...
{
  fprintf(stderr, "get_address, ss = %p, local = %d, dgram = %d, family = %d, name = %s\n", ss, local, dgram, family, name);
...
main (int argc, char **argv)
{
  ...
  while ((opt = getopt_long (argc, argv, "cdust:w:l", o, NULL)) != -1)
    switch (opt) {
    case 'c':
      fprintf(stderr, "opt_client = 1\n");
      opt_client = 1;
      break;
    case 'd':
      fprintf(stderr, "opt_debug = 1\n");
      opt_debug = 1;
      break;
    case 'l':
      {
  ...
  if (opt_server) {
    ...
    cn->rfd = 0;
    cn->wfd = 1;
    fprintf(stderr, "else opt_client==0 && opt_server==0\n");
